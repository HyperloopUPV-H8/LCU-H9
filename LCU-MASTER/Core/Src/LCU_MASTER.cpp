...
void LCU::state_machine_definition(){
	...
	generalStateMachine.add_low_precision_cyclic_action([&](){lcu_instance->commflags.noise_data_to_send = true;}, chrono::milliseconds(ETH_REFRESH_DATA_PERIOD_MS), {OPERATIONAL, FAULT});
...
void LCU::check_communications(){
	...
	if(commflags.noise_data_to_send){
		Communication::send_noise_data_to_backend();
		commflags.noise_data_to_send = false;
	}
}
